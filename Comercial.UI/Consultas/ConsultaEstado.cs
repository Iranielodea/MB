...
namespace Comercial.UI.Consultas
{
    ...
    {
        public Estado Pesquisar(int id, string descricao, int codEmpresa, TipoPesquisaGeral tipoPesquisa)
        {
            ...
            if (tipoPesquisa == TipoPesquisaGeral.pgTela)
            {
                ...
                if (formulario.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        return unitOfWork.ServicoEstado.ObterPorId(DadosStaticos.IdSelecionado);
                    }
                }
            }
            ...
            if (tipoPesquisa == TipoPesquisaGeral.pgId && id > 0)
            {
                using (var unitOfWork = ObjectFactory.GetInstance<IUnitOfWork>())
                {
                    var model = unitOfWork.ServicoEstado.ObterPorId(id);
                    if (model == null)
            ...
            if (tipoPesquisa == TipoPesquisaGeral.pgDescricao && descricao.Length > 0)
            {
                using (var unitOfWork = ObjectFactory.GetInstance<IUnitOfWork>())
                {
                    var model = unitOfWork.ServicoEstado.Filtrar("DESC_ESTADO", descricao, codEmpresa);
                    if (model == null)
                    {
                        frmEstados formulario = new frmEstados();
                        if (formulario.ShowDialog() == DialogResult.OK)
                    ...
                    {
                        if (model.Count() == 1)
                            return unitOfWork.ServicoEstado.ObterPorId(model.First().Id_Estado);
                        else
                        {
                            frmEstados formulario = new frmEstados(descricao);
                            if (formulario.ShowDialog() == DialogResult.OK)
                            {
                                return unitOfWork.ServicoEstado.ObterPorId(DadosStaticos.IdSelecionado);
                            }
